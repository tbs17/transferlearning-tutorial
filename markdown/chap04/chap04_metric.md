...
<img src="https://render.githubusercontent.com/render/math?math=DISTANCE(\mathcal{D}_s,\mathcal{D}_t) = \mathrm{DistanceMeasure}(\cdot,\cdot)">
<!-- $$ -->
...
<img src="https://render.githubusercontent.com/render/math?math=d_{Euclidean}=\sqrt{(\mathbf{x}-\mathbf{y})^\top (\mathbf{x}-\mathbf{y})}">
<!-- $$
...
<img src="https://render.githubusercontent.com/render/math?math=d_{Minkowski}=(||\mathbf{x}-\mathbf{y}||^p)^{1/p}">
<!-- $$
...
<img src="https://render.githubusercontent.com/render/math?math=d_{Mahalanobis}=\sqrt{(\mathbf{x}-\mathbf{y})^\top \Sigma^{-1} (\mathbf{x}-\mathbf{y})}">
<!-- $$
...
<img src="https://render.githubusercontent.com/render/math?math=\cos (\mathbf{x},\mathbf{y}) = \frac{\mathbf{x} \cdot \mathbf{y}}{|\mathbf{x}|\cdot |\mathbf{y}|}">
<!-- $$
...
<img src="https://render.githubusercontent.com/render/math?math=I(X;Y)=\sum_{x \in X} \sum_{y \in Y} p(x,y) \log \frac{p(x,y)}{p(x)p(y)}">
<!-- $$
...
对于两个矩阵$$\mathbf{X},\mathbf{Y}$$，计算方法：首先正交化(用PCA)两个矩阵，然后：<img src="https://render.githubusercontent.com/render/math?math=PA(\mathbf{X},\mathbf{Y})=\sum_{i=1}^{\min(m,n)} \sin \theta_i">
<!-- $$
PA(\mathbf{X},\mathbf{Y})=\sum_{i=1}^{\min(m,n)} \sin \theta_i
$$
 -->
其中$$m,n$$分别是两个矩阵的维度，$$\theta_i$$是两个矩阵第$$i$$个维度的夹角，<img src="https://render.githubusercontent.com/render/math?math=\Theta=\{\theta_1,\theta_2,\cdots,\theta_t\}">是两个矩阵SVD后的角度：<img src="https://render.githubusercontent.com/render/math?math=\mathbf{X}^\top\mathbf{Y}=\mathbf{U} (\cos \Theta) \mathbf{V}^\top">
...
<img src="https://render.githubusercontent.com/render/math?math=\mathcal{A}(\mathcal{D}_s,\mathcal{D}_t) = 2(1 - 2 err(h))">
<!-- $$
...
<img src="https://render.githubusercontent.com/render/math?math=HSIC(X,Y) = trace(HXHY)">
<!-- $$
	HSIC(X,Y) = trace(HXHY)
$$ -->
...
<img src="https://render.githubusercontent.com/render/math?math=W_p(\mathbb{P}, \mathbb{Q}) = \Big(\inf_{\mu \in \Gamma(\mathbb{P}, \mathbb{Q}) } \int \rho(x,y)^p d\mu(x,y) \Big)^{1/p}">
<!-- $$
...
<img src="https://render.githubusercontent.com/render/math?math=W_1(\mathbb{P},\mathbb{Q})= \sup_{\left \| f \right \|_L \leq 1} \mathbb{E}_{x \sim \mathbb{P}}[f(x)] - \mathbb{E}_{x \sim \mathbb{Q}}[f(x)],">

<!-- $$
W_1(\mathbb{P},\mathbb{Q})= \sup_{\left \| f \right \|_L \leq 1} \mathbb{E}_{x \sim \mathbb{P}}[f(x)] - \mathbb{E}_{x \sim \mathbb{Q}}[f(x)],
$$ -->

其中<img src="https://render.githubusercontent.com/render/math?math=\left \| f \right \|_L = \sup{|f(x) - f(y)|} / \rho(x,y)">并且 <img src="https://render.githubusercontent.com/render/math?math=\left \| f \right \|_L = \sup{|f(x) - f(y)|} / \rho(x,y)">称为<img src="https://render.githubusercontent.com/render/math?math=1-">利普希茨条件。
